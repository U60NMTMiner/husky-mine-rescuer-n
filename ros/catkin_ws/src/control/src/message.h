...
namespace message {
    ...
    differentialControl(uint8_t* msg,
        double p,
        double i,
        double d,
        double feedfwd,
    ...
    differentialOutput(uint8_t* msg, double left, double right);
    differentialSpeed(uint8_t* msg,
        double left_spd,
        double right_speed,
        double left_accel,
        double right_accel);
    maxAccel(uint8_t* msg, double max_fwd, double max_rev);
    maxSpeed(uint8_t* msg, double max_fwd, double max_rev);
    platformName(uint8_t* msg, const char *name);
    platformTime(uint8_t* msg, uint32_t time);
    safetySystem(uint8_t* msg, uint16_t flags);
    turn(uint8_t* msg, double trans, double rad, double accel);
    velocity(uint8_t* msg, double trans, double rot, double accel);
}
